...
<body onload="printClock()">
	...
	<script type="text/javascript">
		...
		var table = $('#taable').DataTable({
			...
			ajax : {
				//내가 결재해야할 신청건에 대한 리스트 받기 ({path}로 받음)
				url : "/E_BLOCK_plus/esti/list/list/submit", //"./esti_fromMe_JSON.jsp",
	...
	<form id="f_esmt">
		<div class="ui tiny modal" id="modal_allUpd">
			...
			<div class="ui segment" style="margin-right: 20px; margin-left: 20px">
				<div class="ui form">
					<div class="field">
						<label>기준 가격</label> <input type="text" id="esti_price" placeholder="아래 비율을 적용할 기준단가를 입력해주세요.">
					</div>
				</div>
				...
				<div class="ui form">
					<div class="two fields">
						<div class="field">
							<label>기준가격 이상 변동 비율</label> <input type="text" id="esti_ratio_up" placeholder="'%'를 빼고 입력홰주세요.">
						</div>
						<div class="field">
							<label>기준가격 미만 변동 비율</label> <input type="text" id="esti_ratio_dn" placeholder="'%'를 빼고 입력홰주세요.">
						</div>
					</div>
				</div>
			</div>
	...
	<script type="text/javascript">
		$("#div_allUpd").on('click', function() {
			...
			$("#f_esmt").attr("action", "./esti_result.jsp"); //일괄수정 신청하기 프로시저 매핑
			$("#f_esmt").submit();
			alert("신청되었습니다.");
		});
	</script>
