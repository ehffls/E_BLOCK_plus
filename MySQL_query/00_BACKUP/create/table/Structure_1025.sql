...
/*!50003 CREATE DEFINER=`EBlockMaster`@`%` PROCEDURE `proc_cntr_addEmp`(
     -- emp table
     IN _e_no		NUMERIC(6)
    ,IN _d_no 		NUMERIC(4)
    ,IN _e_id		VARCHAR(30)
...
BEGIN -- 안쪽 구문
	...
	-- 0일때 신입사원 등록
		...
		-- emp 테이블에 INSERT 실행
		INSERT INTO `emp`(e_no, e_id, e_pw, e_name, e_jdate, au_no,
				  e_ph, e_email, e_addr, note, e_bdate, gender, e_rank)            
		VALUES(_new_eno, _e_id, _e_pw, _e_name, _e_jdate, _au_no,
...
/*Table structure for table `view_inb_eq_detail` */

DROP TABLE IF EXISTS `view_inb_eq_detail`;

/*!50001 DROP VIEW IF EXISTS `view_inb_eq_detail` */;
/*!50001 DROP TABLE IF EXISTS `view_inb_eq_detail` */;

/*!50001 CREATE TABLE  `view_inb_eq_detail`(
 `inb_no` varchar(6) ,
 `eq_no` decimal(8,0) ,
 `eq_name` varchar(50) ,
 `eq_sort` varchar(5) ,
...
/*Table structure for table `view_retire_ask_list` */

DROP TABLE IF EXISTS `view_retire_ask_list`;

/*!50001 DROP VIEW IF EXISTS `view_retire_ask_list` */;
/*!50001 DROP TABLE IF EXISTS `view_retire_ask_list` */;

/*!50001 CREATE TABLE  `view_retire_ask_list`(
 `rt_no` decimal(6,0) ,
 `rt_date` varchar(10) ,
 `ask_eno` decimal(6,0) ,
 `ask_rsn` varchar(500) ,
...
/*View structure for view view_inb_eq_detail */

/*!50001 DROP TABLE IF EXISTS `view_inb_eq_detail` */;
/*!50001 DROP VIEW IF EXISTS `view_inb_eq_detail` */;

/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`EBlockMaster`@`%` SQL SECURITY DEFINER VIEW `view_inb_eq_detail` AS select `inb`.`inb_no` AS `inb_no`,`inb`.`eq_no` AS `eq_no`,`eq`.`eq_name` AS `eq_name`,`eq`.`eq_sort` AS `eq_sort`,(case `inb`.`inb_state` when 'inb-0' then '대여중' when 'inb-1' then '신청가능' when 'inb-2' then '분실' when 'inb-3' then '파손' when 'inb-4' then '수리중' when 'inb-5' then '반입완료' when 'inb-6' then '폐기' else 'ERROR' end) AS `inb_state`,`iol`.`sign_date` AS `last_date`,`iol`.`ask_eno` AS `last_eno`,`e`.`e_name` AS `last_ename`,`d`.`d_name` AS `last_dname` from (((`inb_io_list` `iol` left join (`inb_eq` `inb` left join `eq` on((`inb`.`eq_no` = `eq`.`eq_no`))) on((`iol`.`inb_no` = `inb`.`inb_no`))) left join `emp` `e` on((`e`.`e_no` = `iol`.`ask_eno`))) left join `dept` `d` on((`d`.`d_no` = `e`.`d_no`))) order by `iol`.`sign_date` desc,`inb`.`inb_no` */;
...
/*View structure for view view_retire_ask_list */

/*!50001 DROP TABLE IF EXISTS `view_retire_ask_list` */;
/*!50001 DROP VIEW IF EXISTS `view_retire_ask_list` */;

/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`EBlockMaster`@`%` SQL SECURITY DEFINER VIEW `view_retire_ask_list` AS select `retire_ask_list`.`rt_no` AS `rt_no`,`retire_ask_list`.`rt_date` AS `rt_date`,`retire_ask_list`.`ask_eno` AS `ask_eno`,`retire_ask_list`.`ask_rsn` AS `ask_rsn`,`retire_ask_list`.`sign_eno` AS `sign_eno`,`retire_ask_list`.`sign_rsn` AS `sign_rsn`,(case `retire_ask_list`.`outcome` when 'rta-0' then '취소' when 'rta-1' then '대기' when 'rta-2' then '기각' when 'rta-3' then '승인' else 'ERROR' end) AS `outcome`,`retire_ask_list`.`ask_date` AS `ask_date`,`retire_ask_list`.`sign_date` AS `sign_date` from `retire_ask_list` order by (case when (`retire_ask_list`.`outcome` = '취소') then 0 when (`retire_ask_list`.`outcome` = '대기') then 1 when (`retire_ask_list`.`outcome` = '기각') then 2 when (`retire_ask_list`.`outcome` = '승인') then 3 else 4 end) */;
